    ...
    try:
        ...
        for term in EGP_SEARCH_TERMS:
            try:
                ...
                # Extract rows, cells, and specific link attributes safely via DOM evaluation
                tenders_data = page.evaluate("""() => {
                    const rows = Array.from(document.querySelectorAll("table tbody tr, .table tr, tr[role='row']"));
                    return rows.map(row => {
                ...
                print(f"🔍 eGP Search term '{term}': Extracted {len(tenders_data)} table rows via DOM.", flush=True)
                ...
                for item in tenders_data:
                    ...
                    ref_no = cells[0] if len(cells) > 0 else "N/A"
                    
                    potential_titles = [c for c in cells if len(c) > 10]
                    title = max(potential_titles, key=len) if potential_titles else row_text

                    # Filter links to find the specific detail link (avoiding /bids/all or empty/hash anchors)
                    full_link = EGP_BIDS_URL
                    for l in links:
                        h = l["href"]
                        if h and "bids/all" not in h and not h.endswith("/bids") and not h.endswith("#"):
                            full_link = h
                            break
